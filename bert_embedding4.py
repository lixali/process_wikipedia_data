...
#                             if full == True: break
#                             if word in sentence:
#                                 sentences[word].append(sentence)  ### changed by lixiang; this is a list of sentences
    ...
    for word in token_vecs_sum:
        ...
        for idx, sentence in enumerate(token_vecs_sum[word]):
            for i, indiv_word_vec in enumerate(sentence):
                ...
                # if tokenized_text[word][idx][i] == word or (i+1 < len(tokenized_text[word][idx]) and (tokenized_text[word][idx][i].replace("#","") + tokenized_text[word][idx][i+1].replace("#","") == word)):
                    wordcount += 1
